...
class Database {
    ...
    private function connect() {
        ...
        // Accept legacy DB_HOST values like "127.0.0.1:3306", but prefer DB_PORT.
        ...
        if ($is_x10hosting) {
            ...
            $portSegment = $port > 0 ? ';port=' . $port : '';
            $dsn = 'mysql:host=' . $host . $portSegment . ';dbname=' . $this->dbname;
            $options = array(
                PDO::ATTR_PERSISTENT => false,
                PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
                PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8mb4",
        ...
        try {
            if ($is_x10hosting) {
                ...
                $tempdb = new PDO('mysql:host=' . $host . $portSegment, $this->user, $this->pass);
