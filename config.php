...
if ($is_local_development) {
    ...
    define('DB_PORT', (int) getConfigValue($config_overrides, 'DB_PORT', 3306));
    ...
    define('DB_HOST', getConfigValue($config_overrides, 'DB_HOST', '127.0.0.1'));
    define('DB_PORT', (int) getConfigValue($config_overrides, 'DB_PORT', 3306));
...
$normalized_document_root = $document_root !== '' ? str_replace('\\', '/', realpath($document_root) ?: $document_root) : '';
$normalized_app_root = str_replace('\\', '/', realpath(APP_ROOT) ?: APP_ROOT);
$local_base_path = '';

if ($normalized_document_root !== '') {
    $normalized_document_root = rtrim($normalized_document_root, '/');
    if ($normalized_app_root === $normalized_document_root) {
        $local_base_path = '';
    } elseif (strpos($normalized_app_root, $normalized_document_root . '/') === 0) {
        $local_base_path = substr($normalized_app_root, strlen($normalized_document_root));
    }
}

if ($local_base_path === '' && strpos($normalized_app_root, '/htdocs/') !== false) {
    $local_base_path = substr($normalized_app_root, strpos($normalized_app_root, '/htdocs/') + strlen('/htdocs'));
}

$local_base_path = '/' . trim($local_base_path, '/');
if ($local_base_path === '/') {
    $local_base_path = '';
}

define('URL_SUBFOLDER', $local_base_path);

// Dynamic URL_ROOT based on environment
if ($is_local_development) {
    // Development environment
    $local_host = $current_url !== '' ? $current_url : 'localhost';
    define('URL_ROOT', rtrim($protocol . $local_host . URL_SUBFOLDER, '/'));
} else {
    // Production environment
